...
public class LogueoUsuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            String googleToken = request.getParameter("google_token");
            if (googleToken != null) {
                // Validar token de Google
                com.google.api.client.googleapis.auth.oauth2.GoogleIdToken.Payload payload = util.GoogleToken.validar(googleToken);
                if (payload != null) {
            ...
            // --- NUEVO: Validar token de Facebook ---
            String facebookToken = request.getParameter("facebook_token");
            if (facebookToken != null) {
                // Validar token de Facebook
                String email = util.FacebookToken.validar(facebookToken);
                if (email != null) {
                    String token = util.JwtUtil.generarToken(email);
                    out.println("{\"resultado\":\"ok\",\"token\":\"" + token + "\"}");
                } else {
